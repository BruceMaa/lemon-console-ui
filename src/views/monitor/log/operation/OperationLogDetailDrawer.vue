<template>
  <a-drawer v-model:visible="visible" title="日志详情" :width="width >= 720 ? 720 : '100%'" :footer="false">
    <a-descriptions title="基本信息" :column="2" size="large" class="general-description">
      ...
      <a-descriptions-item label="操作人">{{ dataDetail?.createdUsername }}</a-descriptions-item>
      ...
      <a-descriptions-item label="耗时">
        <a-tag v-if="(dataDetail?.timeTaken ?? 0) > 500" color="red"> {{ dataDetail?.timeTaken }}ms </a-tag>
        <a-tag v-else-if="(dataDetail?.timeTaken ?? 0) > 200" color="orange"> {{ dataDetail?.timeTaken }}ms </a-tag>
...
import type { LogDetailResp } from '@/apis'
import { useWindowSize } from '@vueuse/core'
import { getLog as getDetail } from '@/apis'
