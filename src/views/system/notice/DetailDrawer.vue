<template>
  <a-drawer v-model:visible="visible" title="公告详情" :width="width >= 500 ? 500 : '100%'" :footer="false">
    <a-descriptions :column="2" size="large" class="general-description">
      ...
      <a-descriptions-item label="创建人">{{ dataDetail?.createdUsername }}</a-descriptions-item>
      <a-descriptions-item label="创建时间">{{ dataDetail?.createdAt }}</a-descriptions-item>
      <a-descriptions-item label="修改人">{{ dataDetail?.modifiedUsername }}</a-descriptions-item>
      <a-descriptions-item label="修改时间">{{ dataDetail?.modifiedAt }}</a-descriptions-item>
    </a-descriptions>
  </a-drawer>
</template>
...
import { getNotice as getDetail } from '@/apis'
