  ...
  <GiPageLayout>
    ...
    >
      ...
      <template #toolbar-right>
        <a-button v-permission="['system:menus:create']" type="primary" @click="onAdd()">
          <template #icon><icon-plus /></template>
          <template #default>新增</template>
        </a-button>
        <a-button v-permission="['system:menus:clearCache']" type="outline" status="warning" @click="onClearCache">
      ...
      <template #action="{ record }">
        <a-space>
          <a-link v-permission="['system:menus:update']" title="修改" @click="onUpdate(record)">修改</a-link>
          <a-link v-permission="['system:menus:delete']" status="danger" title="删除" @click="onDelete(record)">删除</a-link>
          <a-link
...
import type { MenuResp, MenuTreeResp } from '@/apis/system/menu'
...
const searchData = (title: string, path: string, permission: string) => {
  const loop = (data: MenuTreeResp[]) => {
    const result = [] as MenuTreeResp[]
    data.forEach((item: MenuTreeResp) => {
      if (
...
const columns: TableInstance['columns'] = [
  ...
  { title: '创建人', dataIndex: 'createdUsername', ellipsis: true, tooltip: true, show: false },
  { title: '创建时间', dataIndex: 'createdAt', width: 180 },
  { title: '修改人', dataIndex: 'modifiedUsername', ellipsis: true, tooltip: true, show: false },
  { title: '修改时间', dataIndex: 'modifiedAt', width: 180, show: false },
  {
    title: '操作',
    dataIndex: 'action',
    slotName: 'action',
    ...
    show: has.hasPermOr(['system:menus:update', 'system:menus:delete', 'system:menus:create']),
  },
]
