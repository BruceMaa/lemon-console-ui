...
import type { ClientQuery, ClientResp } from '@/apis'
import { deleteClient, listClient } from '@/apis'
...
import { DisEnableStatusList } from '@/constants/common'
...
const formatAuthType = (data: string[]) => {
  return data.map((item: string) => {
    const authType = auth_type_enum.value?.find((d: LabelValue) => d.value === item)
    return authType?.label || item
  })
}
