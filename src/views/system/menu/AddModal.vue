      ...
      <a-row>
        ...
        <a-col v-bind="colProps">
          ...
          <a-form-item v-else label="权限标识" field="permission">
            <a-input v-model="form.permission" placeholder="system:users:add" :max-length="100" show-word-limit allow-clear />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row>
...
import type { MenuTreeResp } from '@/apis'
...
const menuSelectTree = computed(() => {
  const menus = JSON.parse(JSON.stringify(props.menus)) as MenuTreeResp[]
