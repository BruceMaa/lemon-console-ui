  ...
  <GiPageLayout>
    ...
    >
      ...
      <template #action="{ record }">
        <a-space>
          <a-link v-permission="['system:notices:get']" title="详情" @click="onDetail(record)">详情</a-link>
          <a-link v-permission="['system:notices:view']" title="预览" @click="onView(record)">查看</a-link>
          <a-dropdown>
            <a-button v-if="has.hasPermOr(['system:notices:update', 'system:notices:delete'])" type="text" size="mini" title="更多">
              <template #icon>
                <icon-more :size="16" />
              </template>
            </a-button>
            <template #content>
              <a-doption v-permission="['system:notices:update']">
                <a-link title="修改" @click="onUpdate(record)">修改</a-link>
              </a-doption>
              <a-doption v-permission="['system:notices:delete']">
...
import type { NoticeMethod } from '@/enums'
...
const columns: TableInstance['columns'] = [
  ...
  { title: '发布人', dataIndex: 'createdUsername', maxWidth: 120, ellipsis: true, tooltip: true },
  ...
  { title: '是否置顶', dataIndex: 'isTop', slotName: 'isTop', show: false, maxWidth: 100, align: 'center' },
  {
    ...
    show: has.hasPermOr(['system:notices:get', 'system:notices:view', 'system:notices:update', 'system:notices:delete']),
  },
]
...
const formatNoticeMethods = (noticeMethods: NoticeMethod[]) => {
  return noticeMethods.map((method) => {
    const dictItem = notice_method_enum.value.find((item) => item.value === String(method))
    return dictItem?.label || String(method)
  })
}
</script>
