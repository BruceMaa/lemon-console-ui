...
export function listFile(query: T.FilePageQuery) {
  return http.get<PageRes<T.FileResp[]>>(`${BASE_URL}`, query)
}
...
export function checkFile(sha256: string) {
  return http.get<T.FileResp>(`${BASE_URL}/check`, { fileHash: sha256 })
}

/** @desc 创建文件夹 */
export function createDir(parentPath: string, name: string) {
  return http.post(`${BASE_URL}/dir`, { parentPath, originalName: name })
}

/** @desc 查询文件夹大小 */
export function calcDirSize(id: string) {
  return http.get(`${BASE_URL}/dir/${id}/size`)
}
