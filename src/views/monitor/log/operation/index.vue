<template>
  ...
  >
    <template #toolbar-left>
      <a-input-search v-model="queryForm.createdUsername" placeholder="搜索操作人" allow-clear @search="search" />
      <a-input-search v-model="queryForm.ip" placeholder="搜索操作 IP 或地点" allow-clear @search="search" />
      <DateRangePicker v-model="queryForm.createdAt" @change="search" />
    ...
    <template #toolbar-right>
      <a-button v-permission="['monitor:logs:export']" @click="onExport">
        <template #icon><icon-download /></template>
        <template #default>导出</template>
      </a-button>
    </template>
    <template v-if="has.hasPermOr(['monitor:logs:get'])" #createdAt="{ record }">
      <a-link @click="onDetail(record)">{{ record.createdAt }}</a-link>
    </template>
...
import type { LogQuery, LogResp } from '@/apis'
import dayjs from 'dayjs'
import { exportOperationLog, listLog } from '@/apis'
...
const columns: TableInstance['columns'] = [
  ...
  { title: '操作时间', dataIndex: 'createdAt', slotName: 'createdAt', width: 180 },
  { title: '操作人', dataIndex: 'createdUsername', ellipsis: true, tooltip: true },
...
const reset = () => {
  ...
  queryForm.createdUsername = undefined
  queryForm.createdAt = [
    dayjs().subtract(6, 'day').startOf('day').format('YYYY-MM-DD HH:mm:ss'),
    dayjs().endOf('day').format('YYYY-MM-DD HH:mm:ss'),
  ]
