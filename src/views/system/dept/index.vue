...
import type { DeptQuery, DeptTreeResp } from '@/apis'
import type GiTable from '@/components/GiTable/index.vue'
import { Vue3TreeOrg } from 'vue3-tree-org'
import { deleteDept, exportDept, listDept } from '@/apis'
...
const searchData = (name: string) => {
  const loop = (data: DeptTreeResp[]) => {
    const result = [] as DeptTreeResp[]
    data.forEach((item: DeptTreeResp) => {
...
const columns: TableInstance['columns'] = [
  ...
  { title: '修改时间', dataIndex: 'modifiedAt', width: 180, show: false },
  {
    ...
    show: has.hasPermOr(['system:depts:update', 'system:depts:delete', 'system:depts:create']),
  },
]
...
// 删除
const onDelete = (record: DeptTreeResp) => {
...
const handleAdd = (record: DeptTreeResp) => {
  onAdd(record.id)
}
// 修改
const onUpdate = (record: DeptTreeResp) => {
