    ...
    >
      <template #columns>
        <a-table-column title="名称">
          <template #cell="{ record }">
            ...
            >
              ...
              <template v-if="has.hasPermOr(['system:files:update', 'system:files:get', 'system:files:download', 'system:files:delete'])" #content>
        ...
        <a-table-column title="大小" data-index="size" :width="160">
          <template #cell="{ record }">
            <span v-if="record.type === 0" v-permission="['system:files:calcDirSize']">
        ...
        <a-table-column v-if="has.hasPermOr(['system:files:update', 'system:files:get', 'system:files:download', 'system:files:delete'])" title="操作" :width="120" align="center">
          <template #cell="{ record }">
...
const emit = defineEmits<{
  (e: 'click', record: FileResp): void
  (e: 'dblclick', record: FileResp): void
  (e: 'select', record: FileResp): void
  (e: 'right-menu-click', mode: string, item: FileResp): void
}>()
...
// 计算文件夹大小
const calculateDirSize = async (record: FileResp) => {
...
const select: TableInstance['onSelect'] = (rowKeys, rowKey, record) => {
  emit('select', record as unknown as FileResp)
}

// 单击事件
const handleClick = (record: FileResp) => {
  emit('click', record)
}
...
// 右键菜单点击事件
const handleRightMenuClick = (mode: string, item: FileResp) => {
