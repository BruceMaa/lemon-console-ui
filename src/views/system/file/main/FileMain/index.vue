    ...
    <a-row justify="space-between" class="file-main__search">
      ...
      <a-space wrap>
        ...
        <a-dropdown trigger="click">
          ...
          <template #content>
            <!-- 普通上传 -->
            <a-upload v-permission="['system:files:upload']" :show-file-list="false" :custom-request="handleUpload" style="display: block;">
      ...
      <a-space wrap>
        ...
        <a-button v-permission="['system:files:createDir']" type="primary" :disabled="!queryForm.parentPath" @click="createDirModalVisible = !createDirModalVisible">
          <template #icon>
            <icon-folder />
          </template>
          <template #default>新建文件夹</template>
        </a-button>
        <a-button v-permission="['system:files:delete']" type="primary" @click="isBatchMode = !isBatchMode">
          <template #icon>
            <icon-select-all />
          </template>
...
import type { FileQuery, FileResp } from '@/apis'
import type { ExcelConfig } from '@/components/FilePreview/type'
import { Message, Modal } from '@arco-design/web-vue'
import { useWindowSize } from '@vueuse/core'
import { api as viewerApi } from 'v-viewer'
import { createDir, deleteFile, listFile, uploadFile } from '@/apis'
import { ImageTypes, OfficeTypes } from '@/constants/file'
...
const queryForm = reactive<FileQuery>({
  ...
  type: route.query.type?.toString() && route.query.type?.toString() !== '0' ? Number(route.query.type) : undefined,
  sort: ['type,asc', 'modifiedAt,desc'],
})
...
// 点击文件
const handleClickFile = (item: FileResp) => {
...
// 双击文件
const handleDblclickFile = (item: FileResp) => {
  if (item.type === 0) {
...
// 下载文件
const onDownload = async (fileInfo: FileResp) => {
...
// 右键菜单
const handleRightMenuClick = async (mode: string, fileInfo: FileResp) => {
  if (mode === 'delete') {
    Modal.warning({
      title: '提示',
...
// 勾选文件
const handleSelectFile = (item: FileResp) => {
...
onBeforeRouteUpdate((to) => {
  ...
  if (to.query.type === '0' || !to.query.type) {
    ...
    queryForm.type = Number(to.query.type)
